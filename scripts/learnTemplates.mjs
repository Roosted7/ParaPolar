// HTML templates for the pre-rendered /learn pages. Shared by the build
// script (scripts/build-learn.mjs) and the Vite dev middleware, so /learn
// works identically on the dev server and in production.
...
const CHROME = {
  en: {
    learn: "Learn",
    lessonsCta: "Prefer doing over reading? Take the six interactive lessons →",
  ...
  de: {
    learn: "Lernen",
    lessonsCta: "Lieber ausprobieren statt lesen? Zu den sechs interaktiven Lektionen →",
  ...
  fr: {
    learn: "Apprendre",
    lessonsCta: "Plutôt pratique que lecture ? Suivez les six leçons interactives →",
  ...
  nl: {
    learn: "Leren",
    lessonsCta: "Liever doen dan lezen? Volg de zes interactieve lessen →",
...
<main>
  ...
  <div class="more"><a href="${prefix(lang)}/?lesson=1">${esc(c.lessonsCta)}</a></div>
</main>
<footer>PARAPOLAR · ${esc(c.footer)}</footer>`;
  return pageShell({
    lang,
...
/** All learn routes as a Map of "/de/learn/speed-to-fly/" -> HTML. */
export function renderLearnRoutes() {
  const routes = new Map();
  for (const lang of LANGS) {
    const articles = LEARN_ARTICLES[lang];
    routes.set(`${prefix(lang)}/learn/`, indexHtml(lang, articles));
    for (const article of articles) {
      const siblings = articles.filter((a) => a.slug !== article.slug);
      routes.set(`${prefix(lang)}/learn/${article.slug}/`, articleHtml(lang, article, siblings));
    }
  }
  return routes;
}
