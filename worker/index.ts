...
<style>
  :root { --ink:#0e1a2b; --deep:#0a1524; --soft:#16233a; --glacier:#edf1f4; --thermal:#e8833a;
    --bright:#f5b46b; --line:rgba(255,255,255,0.14); --dim:#8ba0b3; }
  * { box-sizing:border-box; }
  ...
  .row .k { width:180px; color:var(--dim); overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }
  ...
  .row .n { width:70px; text-align:right; font-variant-numeric:tabular-nums; }
...
(async () => {
  ...
  try {
    ...
    if (json.error) {
      out.innerHTML = '<section class="err">Not available yet: ' + json.error +
        '.<br/>Create an API token (Account Analytics: Read) and run <code>wrangler secret put ANALYTICS_API_TOKEN</code>.</section>';
      return;
    }
    ...
    const bars = (rs, label) => {
      ...
      return rs.map(r =>
        '<div class="row"><span class="k">' + esc(r.detail || label || '—') + '</span>' +
        '<span class="bar"><i style="width:' + Math.max(2, 100 * Number(r.count) / max) + '%"></i></span>' +
    ...
    const avgScore = flights.length
      ? flights.reduce((a, r) => a + Number(r.avg_value) * Number(r.count), 0) / sum(flights)
      : null;
    ...
    out.innerHTML =
      ...
      '<section><h2>Valley challenge</h2><div class="big">' + sum(flights).toLocaleString() +
        ' <small>flights</small>' +
        (avgScore != null && sum(flights) > 0 ? ' &nbsp; ' + avgScore.toFixed(0) + '<small>/100 avg score</small>' : '') +
        '</div></section>' +
      ...
      '</section>' +
      '<div class="sub">Traffic &amp; page views live in Cloudflare Web Analytics. <a href="/">Back to the app →</a></div>';
  } catch (e) {
